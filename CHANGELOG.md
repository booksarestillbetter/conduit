...
- **Unauthenticated DB-write / notification-spam endpoint (`src/api/sync_routes.rs`)**: `POST /api/sync/notify-download` had no auth check at all, despite the post-download hook script itself already sending an `X-Api-Key` header for exactly this purpose — anyone able to reach the port could write arbitrary event-log rows, overwrite real Arr grab records, and trigger outbound notification spam. Now gated behind the standard `RequireAuth` extractor (JWT, API token, Basic auth, or `X-Api-Key` — whichever the caller already has).
- **Unauthenticated webhook ingestion — Bazarr, Overseerr, Jellyfin (`src/api/webhook_routes.rs`, `src/config/model.rs`)**: unlike every other inbound webhook (Sonarr/Radarr/Lidarr/Plex/Ombi), these three had no secret-verification path and no `webhook_secret` config field to configure one. New `bazarr.webhook_secret` / `overseerr.webhook_secret` / `jellyfin.webhook_secret` config fields, checked via the same constant-time `verify_webhook_secret` helper (and same fail-open-when-unconfigured convention) the other five already use. Overseerr's was the most serious of the three — unauthenticated requests could inject fabricated "media requested/approved" records straight into the pipeline DB.
- **Unauthenticated crash-log endpoint (`src/api/metrics_routes.rs`)**: `GET /api/system/crash-log` returned full panic backtraces and the crash log's absolute file path with no auth, inconsistent with its sibling `/api/system/*` routes. Now `RequireAuth`-gated.
- **Tracker passkeys were leaking into logs (was a Synapse-side fix earlier this session; the equivalent Conduit-side gap was checked for and not found)** — verified `src/notify` and the fetcher crates never log a node's raw API key/password/token, and TLS verification defaults on and is correctly honored across all three REST-backed backends.

### Fixed

- **Fetcher backends were silently no-op'ing (or fabricating a fake success response for) several UI-exposed operations instead of either working or failing loudly** — across `crates/fetcher-transmission`, `crates/fetcher-deluge`, `crates/fetcher-qbittorrent`, `crates/fetcher-synapse`:
  - `replace_trackers`: now implemented for real on Transmission (`trackerList`, the modern non-deprecated `torrent-set` mechanism) and Deluge (`core.set_torrent_trackers`). qBittorrent's was already real. Synapse has no such RPC on its own protocol — this now returns a clear "unsupported" error instead of a fake `Ok(())`.
  - `set_sequential_download`: implemented for real on Transmission (`sequentialDownload`, Transmission 4.1.0+; a safe no-op against older daemons, which ignore unrecognized `torrent-set` arguments) and Deluge (`core.set_torrent_options`). qBittorrent's now actually honors the `enabled` argument instead of unconditionally toggling — it checks each torrent's current `seq_dl` state first and only toggles the ones that need to change. Synapse: honest "unsupported" error, was a fake `Ok(())`.
  - `rename_path`: implemented for real on Deluge (`core.rename_files`, resolving the target file's index via a newly-populated file listing — see below). Synapse: honest "unsupported" error, was a fabricated `{"result": "success"}` response while doing nothing.
  - `set_turtle_mode`: qBittorrent now honors the `enabled` argument the same way as sequential-download above, instead of unconditionally toggling. Deluge has no native alternate-speed concept at all (only raw bandwidth limits, no remembered "alt" pair to swap to) — honest "unsupported" error, was a fake `Ok(())`.
  - `update_blocklist`: qBittorrent, Deluge, and Synapse all genuinely have no built-in blocklist-refresh RPC (this is a Transmission-specific concept) — honest "unsupported" error on all three instead of a fake `Ok(0)`.
  - `queue_move` (Deluge): now issues one batched RPC covering the whole selection instead of one RPC per torrent — both faster and semantically correct for a multi-select reorder (moving torrents one at a time doesn't preserve their relative order the way a real batch move does), and no longer silently swallows a failed call.
  - Deluge's file listing (`files` on `Torrent`) was silently always empty even though the by-hash detail RPC already requested it — `map_deluge_to_torrent` just discarded the response. Now correctly populated from Deluge's `files` + `file_progress` status fields, which is also what makes the `rename_path` fix above actually reachable from the UI's per-file rename control.
- **"Cross-Node Torrent Migration" had no UI entry point** despite being marked complete in `TODO.md` — the backend route and `api.ts` wrapper were both fully implemented and correct, but nothing in the web app ever called it. Added a "Migrate" action to the torrent details modal (target node picker, optional remove-from-source, optional delete-source-data).
- **8 mutating web UI actions never sent the auth header** (`web/src/services/api.ts`): bulk/per-torrent queue-move, sequential-download toggle, rename-path, batch-tracker-replace, turtle-mode toggle (per-node and global), blocklist-update. Each built its `fetch` call with a literal `Content-Type`-only header instead of this file's own `getAuthHeaders()` helper. The backend correctly requires auth on all of them (verified via `RequireAuth` on each handler), so in practice these were live 401 failures on real buttons (e.g. the bulk queue-move buttons, the global turtle-mode toggle) rather than a security hole — but broken all the same. All 8 now use `getAuthHeaders()`.
- **Mobile's queue-move sent the wrong JSON field name** (`mobile/lib/core/network/api_client.dart`): `{"action": direction}` instead of the `{"direction": ...}` the backend's `QueueMovePayload` actually deserializes — every queue-move from the mobile app was silently failing.
- **Mobile stored its auth token in plaintext `SharedPreferences`** (Android: private XML; iOS: plist) instead of the platform keystore — recoverable from an unencrypted device backup or a rooted/jailbroken device. Now stored via `flutter_secure_storage` (Android Keystore / iOS Keychain), with a one-time lazy migration for tokens saved by an older app version so existing users aren't silently logged out on upgrade.
- **7 dead `notify_on_*` config fields** (`notifications.notify_on_grab/download/error/autopurge/replacement/sync/scrobble`) plus `batch_min_count`, all leftover from the pre-0.11 single-target notification model and never read anywhere since (filtering is done entirely per-target via each target's own `categories` list) — removed from `NotificationConfig` and the corresponding TypeScript type, rather than left as config that silently does nothing if someone sets it.
- **`tracker_circuit_breaker.canary_probe_enabled`** — the breaker engine already correctly read this (it gates whether a tripped tracker's recovery is probed early or just waits out its full cooldown), but there was no Settings UI control for it, only a hardcoded default. Added a toggle next to "Auto-resume all on recovery".
- **`enable_registration`** — confirmed this is a genuine forward-looking placeholder for the not-yet-built RBAC/multi-user milestone (`TODO.md`), not an oversight; there's no registration endpoint yet for it to gate. Left as-is, with a doc comment explaining why so it doesn't read as another silent bug.
